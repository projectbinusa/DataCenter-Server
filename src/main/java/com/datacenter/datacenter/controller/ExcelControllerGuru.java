...
@RestController
@RequestMapping("/api/excel-guru")
public class ExcelControllerGuru {
    @Autowired
    ExcelServiceGuru fileService;

    @PostMapping("/upload/guru/{id}")
    public ResponseEntity<?> uploadFile(@PathVariable("id") long id , @RequestParam("file") MultipartFile file) {
        String message = "";
        if (ExcelHelper.hasExcelFormat(file)) {
            try {
                fileService.savee(file,id);
    ...
    @GetMapping("/download-guru/{id}")
    public ResponseEntity<Resource> getFilee(@PathVariable("id") Long id) {
        String filename = "guru.xlsx";
